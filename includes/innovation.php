...
function ensureInnovationSchema() {
    ...
    $columnUpdates = [
        'users' => [
            'avatar_path' => "VARCHAR(255) NULL AFTER email",
        ],
        'noodles' => [
            'image' => "VARCHAR(255) NULL AFTER stock",
        ],
        'rewards' => [
    ...
    if ((int)$conn->query("SELECT COUNT(*) AS count FROM rewards")->fetch_assoc()['count'] === 0) {
        $conn->query("INSERT INTO rewards
            (name,name_zh,description,image_path,points_required,stock,reward_type) VALUES
            ('Free Ajitama Egg','免費溏心蛋','兌換後可於下一碗客製拉麵免費加蛋','assets/images/rewards/reward-ajitama-egg.png',80,50,'topping'),
            ('Partner Tea Voucher','合作店家茶飲券','可至 NEON TEA 兌換限定無糖冷泡茶','assets/images/rewards/reward-partner-tea.png',150,25,'coupon'),
            ('Limited Neon Ramen Bowl','限量霓虹拉麵碗','會員限定收藏碗，限量 12 份','assets/images/rewards/reward-neon-bowl.png',300,12,'limited')");
    }
    ...
    if ((int)$conn->query("SELECT COUNT(*) AS count FROM partners")->fetch_assoc()['count'] === 0) {
        $conn->query("INSERT INTO partners
            ...
            ('NEON TEA','Tea Lab','主打無糖冷泡茶與氣泡飲的合作品牌','拉麵訂單滿 NT$250，茶飲折 NT$20','10:30:00','22:00:00','#00ffff'),
...
function projectUploadImage($field, $folder, $prefix, $currentPath = '') {
    ...
    if (($file['error'] ?? UPLOAD_ERR_OK) !== UPLOAD_ERR_OK) {
        return ['success' => false, 'message' => '圖片上傳失敗，請重新選擇檔案。'];
    }
    if ((int)$file['size'] > 2 * 1024 * 1024) {
        return ['success' => false, 'message' => '圖片請小於 2MB。'];
    }

    $ext = strtolower(pathinfo((string)$file['name'], PATHINFO_EXTENSION));
    $allowed = ['jpg', 'jpeg', 'png', 'webp', 'gif'];
    if (!in_array($ext, $allowed, true)) {
        return ['success' => false, 'message' => '圖片格式請使用 JPG、PNG、WEBP 或 GIF。'];
    }

    $folder = trim($folder, '/');
    $targetDir = __DIR__ . '/../' . $folder;
    if (!is_dir($targetDir) && !mkdir($targetDir, 0775, true)) {
        return ['success' => false, 'message' => '無法建立圖片資料夾。'];
    }

    $safePrefix = preg_replace('/[^A-Za-z0-9-]/', '-', $prefix) ?: 'image';
    $filename = strtolower($safePrefix) . '-' . date('YmdHis') . '-' . bin2hex(random_bytes(3)) . '.' . $ext;
    $destination = $targetDir . '/' . $filename;
    if (!move_uploaded_file((string)$file['tmp_name'], $destination)) {
        return ['success' => false, 'message' => '圖片儲存失敗，請確認資料夾權限。'];
    }

    return ['success' => true, 'path' => $folder . '/' . $filename];
}

function displayImagePath($path, $fallback) {
...
function getDemoLocations() {
    return [
        ...
        ['code' => 'KHH-02', 'name_zh' => '三民建工店', 'name_en' => 'Sanmin Jiangong Store', 'district' => '三民區', 'address' => '建工路 215 號', 'hours' => '06:00-24:00', 'services' => ['語音輔助', 'Wi-Fi', '內用區'], 'machine_status' => '備援機待命', 'ingredient_status' => '辣味加料 42%', 'amenities' => ['智能煮麵機', '配料自動艙', '5G Wi-Fi', '電子票券核銷'], 'today_orders' => 102, 'last_sync' => '31 秒前', 'power_slots' => 5, 'map_query' => '高雄市三民區建工路', 'color' => '#8d7bff'],
        ['code' => 'KHH-03', 'name_zh' => '大樹九大店', 'name_en' => 'Dashu Jiuda Store', 'district' => '大樹區', 'address' => '九大路 36 號', 'hours' => '08:00-22:00', 'services' => ['合作券核銷', '行李櫃', '素食設備'], 'machine_status' => '低噪模式', 'ingredient_status' => '蔬菜艙 64%', 'amenities' => ['智能煮麵機', '素食分流艙', '行李暫存', '強冷空調'], 'today_orders' => 73, 'last_sync' => '45 秒前', 'power_slots' => 3, 'map_query' => '高雄市大樹區九大路', 'color' => '#00ff9d'],
        ['code' => 'KHH-04', 'name_zh' => '湖內東方店', 'name_en' => 'Hunei Dongfang Store', 'district' => '湖內區', 'address' => '東方路 120 號', 'hours' => '24H', 'services' => ['安全候車', '急救箱', '數位支付'], 'machine_status' => '夜間自動巡檢', 'ingredient_status' => '雞湯湯包 91%', 'amenities' => ['24H 全天候自助', '5G Wi-Fi', '安全候車燈箱', '充電座空位'], 'today_orders' => 88, 'last_sync' => '22 秒前', 'power_slots' => 12, 'map_query' => '高雄市湖內區東方路', 'color' => '#ffcf4a'],
        ['code' => 'KHH-05', 'name_zh' => '林園沿海店', 'name_en' => 'Linyuan Yanhai Store', 'district' => '林園區', 'address' => '沿海路 166 號', 'hours' => '24H', 'services' => ['夜間安心連線', '補水', '自行車工具'], 'machine_status' => '強風防護啟用', 'ingredient_status' => '海鮮風味 58%', 'amenities' => ['24H 全天候自助', '智能煮麵機', '防災照明', '自行車工具'], 'today_orders' => 119, 'last_sync' => '27 秒前', 'power_slots' => 6, 'map_query' => '高雄市林園區沿海路', 'color' => '#ff5f7a'],
        ['code' => 'KHH-06', 'name_zh' => '旗津三分店', 'name_en' => 'Cijin 3rd Rd. Store', 'district' => '旗津區', 'address' => '旗津三路 51 號', 'hours' => '09:00-23:00', 'services' => ['寵物友善暫停區', '拍照牆', '沖沙區'], 'machine_status' => '觀光尖峰模式', 'ingredient_status' => '乾拌麵艙 69%', 'amenities' => ['智能煮麵機', '配料自動艙', '拍照牆', '5G Wi-Fi'], 'today_orders' => 135, 'last_sync' => '14 秒前', 'power_slots' => 4, 'map_query' => '高雄市旗津區旗津三路', 'color' => '#ff4dff'],
    ];
}
?>
