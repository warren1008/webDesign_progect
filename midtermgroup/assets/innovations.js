...
(() => {
    ...
    const initFaq = () => {
        ...
        button.innerHTML = '<span aria-hidden="true">?</span><strong data-i18n="faq.title">Smart FAQ</strong>';
        ...
        panel.innerHTML = `
            <div class="faq-panel__header">
                <div><strong data-i18n="faq.title">Smart FAQ</strong><small>DEMO ASSISTANT</small></div>
        ...
        button.addEventListener('click', () => toggle(!panel.classList.contains('is-open')));
        ...
        panel.querySelectorAll('[data-faq]').forEach((question) => question.addEventListener('click', () => {
            ...
            if (key === 'machine' && document.body.dataset.loggedIn === '1' && !panel.querySelector('[data-create-case]')) {
                ...
                caseButton.textContent = text('建立展示用客服案件', 'Create demo support case');
    ...
    const initDashboard = () => {
        ...
        document.querySelector('a[href="checkout.php"]')?.addEventListener('click', () => {
            speak('請至自助機台刷卡或使用示範付款完成結帳，謝謝光臨。', 'Please complete the demo payment at the self-service kiosk. Thank you.');
        });
    };
        ...
        document.querySelectorAll('[data-redeem-reward]').forEach((button) => button.addEventListener('click', async (event) => {
            ...
            try {
                ...
                if (stock) {
                    ...
                    stock.textContent = `${text('餘額', 'Stock')} ${next}`;
                }
    ...
    const initAuth = () => {
        ...
        if (loginForm && !document.querySelector('[data-face-login]')) {
            ...
            faceButton.textContent = text('啟動模擬人臉辨識登入', 'Start Simulated Face ID');
            loginForm.after(faceButton);
            faceButton.addEventListener('click', () => {
                const scan = document.createElement('div');
                scan.className = 'face-scan';
                scan.innerHTML = `<div class="face-grid"><i></i></div><strong>${text('本機 Demo 掃描中，不會啟用或上傳攝影機影像', 'Local demo scan. No camera image is captured or uploaded.')}</strong>`;
        ...
        document.querySelector('[data-fill-demo-email]')?.addEventListener('click', () => {
            ...
            if (email) email.value = 'demo_user@noodles.com';
        });
        if (resetForm) resetForm.addEventListener('submit', (event) => {
            ...
            overlay.innerHTML = `<div class="matrix-terminal"><strong>${text('正在建立安全連線...', 'Establishing secure connection...')}</strong><div><i></i></div><p>${text('正在產生數位簽章 · 郵件加密發送中', 'Generating digital signature · Encrypting demo email')}</p></div>`;
        ...
        randomButton?.addEventListener('click', () => {
            ...
            form.querySelector('[name="password"]').value = 'demo123456';
            form.querySelector('[name="confirm_password"]').value = 'demo123456';
        ...
        if (resetSuccess) {
            ...
            modal({
                ...
                content: `<p>${text('加密重設憑證已成功建立於本機模擬信箱。', 'An encrypted reset credential is ready in the local demo mailbox.')}</p>`,
                actions: [{
                    label: text('開啟 Demo 重設連結', 'Open Demo Reset Link'),
    ...
    const initAdmin = () => {
                    ...
                    actions: [{
                        ...
                        onClick: (shell) => {
                            ...
                            setTimeout(() => {
                                ...
                                shell.closeModal();
                                toast(text('補貨模擬完成，貨架恢復安全庫存。', 'Restock simulation complete.'));
                            }, 1600);
                        }
                    }]
                });
        ...
        if (file === 'analytics.php') {
            ...
            if (insight) {
                ...
                button.addEventListener('click', () => {
                    ...
                    const report = text(
                        '【AI 智慧分析報告】偵測到近期夜間點餐率上升 18%。熱銷商品目前庫存偏低，建議啟動補貨流程，並於明晚低溫時段派發「火辣夜限定」優惠券。此內容為本機規則式展示分析。',
                        '[DEMO ANALYTICS REPORT] Night orders rose 18%. A top-selling item is approaching low stock. Start a restock workflow and schedule the Fire Night offer for tomorrow evening. This is a local rule-based demo analysis.'
                    );
                    output.textContent = '';
                    button.disabled = true;
                    setTimeout(() => {
        ...
        if (file === 'products.php') {
            ...
            if (form) {
                ...
                button.textContent = text('帶入簡報測試泡麵', 'Fill Demo Product');
        ...
        if (file === 'orders.php') {
            ...
            simulate?.addEventListener('click', () => {
                ...
                row.innerHTML = `<td><strong>DEMO-${Date.now().toString().slice(-6)}</strong></td><td>demo_guest<br><small>kiosk@demo.local</small></td><td>${new Date().toLocaleTimeString()}</td><td>2</td><td>$12.44</td><td><span class="status-badge status-paid">Paid</span></td><td><span class="status-badge status-preparing">機器人烹飪中</span></td><td><code>DEMO2026</code></td><td>Demo</td>`;
        ...
        if (file === 'payments.php') {
            document.querySelector('[data-simulate-payment]')?.addEventListener('click', () => {
                ...
                const row = document.createElement('tr');
                row.className = 'row-injected';
                row.innerHTML = `<td><button class="transaction-link" data-transaction="TXN-DEMO">TXN-DEMO</button></td><td>ORD-DEMO</td><td>kiosk_guest</td><td>$12.44</td><td>Visa</td><td><span class="status-badge status-success">Success</span></td><td>${new Date().toLocaleString()}</td>`;
                body?.prepend(row);
    ...
    const initFooter = () => {
        ...
        form?.addEventListener('submit', (event) => {
            ...
            toast(text('已加入模擬通知名單。', 'Added to the simulated alert list.'));
            form.reset();
        });
    };
        ...
        window.setTimeout(() => {
            const content = `
                <div class="home-promo-modal">
                    ...
                    <p>${text('深夜加料優惠、滿額點數加倍與新客展示點數已整合到活動與點數頁。所有優惠皆為 Sandbox 模擬營運。', 'Late-night topping deals, threshold point bonuses and new-member demo points are connected to Deals and Rewards. All offers are sandbox simulations.')}</p>
